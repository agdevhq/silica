...
export async function loadConfig(projectRoot = process.cwd()): Promise<ResolvedSilicaConfig> {
  const configPath = path.join(projectRoot, "silica.config.ts");
  const jsConfigPath = path.join(projectRoot, "silica.config.js");
  const configFile = existsSync(configPath) ? configPath : existsSync(jsConfigPath) ? jsConfigPath : undefined;

  let userConfig: SilicaConfig = {};
  if (configFile) {
    const jiti = createJiti(projectRoot, { interopDefault: true });
    const loaded = await jiti.import<SilicaConfig | { default: SilicaConfig }>(configFile);
...
export function resolveConfig(config: SilicaConfig = {}, projectRoot = process.cwd()): ResolvedSilicaConfig {
  const auth = config.auth === false ? undefined : config.auth;
  const authEnabled = Boolean(auth?.enabled ?? auth?.provider ?? auth?.allowedDomains?.length ?? auth?.allowedEmails?.length);
