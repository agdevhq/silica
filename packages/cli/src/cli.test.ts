...
const MATERIALIZE_TEST_TIMEOUT = 15_000;
...
describe("silica CLI helpers", () => {
  ...
  it(
    "materializes the hidden Next.js app and preserves user public files",
    async () => {
      ...
      expect(
        await fs.pathExists(
          path.join(nextRoot, ".silica-packages", nextTarball),
        ),
      ).toBe(true);
      ...
      expect(
        generatedPackageJson.devDependencies["@types/better-sqlite3"],
      ).toBe(
      ...
      expect(
        await fs.pathExists(path.join(nextRoot, "public/favicon.svg")),
      ).toBe(true);
      expect(
        (
    ...
    MATERIALIZE_TEST_TIMEOUT,
  );

  it(
    "loads project env without materializing env files",
    async () => {
