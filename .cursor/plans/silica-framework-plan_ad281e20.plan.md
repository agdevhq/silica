...
- `proxy.ts` — auth gate on everything except `/api/auth/*` and `/_next/*`. In stateless mode the signed/encrypted cookie itself is the security boundary (verified without a DB call), so the proxy enforces both "signed in" and "in allowlist" directly.
...
### M-1 — Pre-build: secure names + accounts (half day)
...
### M2 — Theme system + default theme (3 days)
